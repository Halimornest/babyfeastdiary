...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if user has babies — redirect to onboarding if not
    ...
    <div className="min-h-screen bg-linear-to-b from-peach-50 via-white to-mint-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Log in to Baby Feast Diary</p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full px-4 py-3 pr-16 bg-cream-50 border-2 border-gray-200 rounded-2xl text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:border-peach-400 focus:ring-2 focus:ring-peach-100 transition-all duration-200"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword((prev) => !prev)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-500 hover:text-peach-600 transition-colors cursor-pointer"
        ...
        <p className="text-center mt-6 text-sm text-gray-500">
          ...
          <Link href="/register" className="text-peach-600 font-semibold hover:text-peach-700 transition-colors">
            Register
          </Link>
        </p>
      </div>
